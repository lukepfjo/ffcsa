...
from ffcsa.shop.utils import set_shipping, set_tax, sign
...
def default_billship_handler(request, order_form):
    ...
    ``ffcsa.shop.utils.set_shipping``. The Cart object is also
...
def default_tax_handler(request, order_form):
    ...
    ``ffcsa.shop.utils.set_tax``. The Cart object is also
    accessible via ``request.cart``
    """
...
def default_payment_handler(request, order_form, order):
    ...
    ffcsa.shop.checkout.CheckoutError("error message") if payment
    is unsuccessful.
    """
    pass
