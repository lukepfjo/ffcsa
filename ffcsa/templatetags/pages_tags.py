from mezzanine.pages.templatetags.pages_tags import *

from ffcsa.shop.models import Category


# This replaces the mezzanine page_menu template tag
@register.render_tag
def page_menu(context, token):
    ...
    for page in context["page_branch"]:
        ...
        # Only display a category if there is a product available or there are sub-categories
        if page.in_menu and hasattr(page, 'category'):
            page.in_menu = page.category.products.filter(available=True).count() > 0 or len(
                context["menu_pages"].get(page.id, [])) > 0
        if page.in_menu:
