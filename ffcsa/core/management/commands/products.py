...
def _product_from_row(row):
    product, created = Product.objects.get_or_create(title=row[TITLE], sku=row[SKU])
    product.content = row[CONTENT]
    product.description = row[DESCRIPTION]
    if product.description:
        product.gen_description = False
...
def import_products(csv_file):
    ...
    for row in reader:
        ...
        if NUM_IN_STOCK in row and row[NUM_IN_STOCK]:
        ...
        if SALE_START_DATE in row and SALE_START_TIME in row and row[SALE_START_DATE] and row[SALE_START_TIME]:
            variation.sale_from = _make_date(row[SALE_START_DATE],
                                             row[SALE_START_TIME])
        if SALE_END_DATE in row and SALE_END_TIME in row and row[SALE_END_DATE] and row[SALE_END_TIME]:
